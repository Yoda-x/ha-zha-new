    ...
    def async_device_initialized(self, device, join):
        ...
        for endpoint_id, endpoint in device.endpoints.items():
            ...
            if endpoint.profile_id in bellows.zigbee.profiles.PROFILES:
                ...
                if zha_const.DEVICE_CLASS.get(endpoint.profile_id,{}).get(endpoint.device_type, None):
			...
			if CONF_IN_CLUSTER in node_config:
				profile_clusters[0] =  profile_clusters[0].update(node_config.get(CONF_IN_CLUSTER,{})
			""" Add aditional allows Out_Clusters from config """
			if CONF_OUT_CLUSTER in node_config:
				profile_clusters[1] =  profile_clusters[1].update(node_config.get(CONF_OUT_CLUSTER,{})
